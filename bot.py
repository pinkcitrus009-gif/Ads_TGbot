...
async def cmd_join(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Join the group game. Auto-copies character from any other chat if available."""
    group_id, user_id = get_ids(update)
    username = get_username(update)
    player   = db.get_player(group_id, user_id)

    copied_from_other = False

    if not player:
        # Пробуем автоматически скопировать персонажа из другого чата
        player, copied_from_other = db.create_player(group_id, user_id, username)

        # Если персонаж так и остался «заготовкой» (имя по умолчанию, 1й уровень, 0 XP)
        # — значит, настоящего персонажа нигде нет, направляем к /start
        if (
            not player
            or (
                player.get('name', 'Герой') == 'Герой'
                and player.get('level', 1) == 1
                and player.get('exp', 0) == 0
                and not copied_from_other
            )
        ):
            await update.message.reply_text(
    ...
    if copied_from_other:
        copy_notice = "🔄 Персонаж перенесён из другого чата!\n\n"
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if player:
        ...
        player, _ = db.create_player(group_id, user_id, username_cb)
...
async def cmd_reset(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Сбросить персонажа в текущем чате. Глобальные сохранения (/saves) НЕ удаляются."""
    group_id, user_id = get_ids(update)
    username = get_username(update)
    db.clear_history(group_id)
    db.reset_player(group_id, user_id, username)
    await update.message.reply_text(
        "🗑 Персонаж и история в этом чате сброшены.\n"
        "Глобальные сохранения (/saves) сохранены — можно загрузить через /load.\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not player:
        player, _ = db.create_player(group_id, user_id, username)
